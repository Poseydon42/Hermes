...
#define VK_CHECK_RESULT(Call) \
	{ \
		...
		{ \
			HERMES_ASSERT_LOG(false, "Vulkan function call failed. Error code is %s", VkResultToString(VkCheckedResult).c_str()); \
			::Hermes::PlatformMisc::ExitWithMessageBox((uint32)-1, "Vulkan error", "One of Vulkan functions returned VkResult code indicating error. Application could not continue normal execution."); \
		} \
	}
